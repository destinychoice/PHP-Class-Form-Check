<?php 
	/* 
		...
		'rules' => 'required,email,number,telephone,mobile,postcode,password',
	*/
	class form_check {
		public $post_data;
		public $error_page;
		...
		function run_validation() {
			...
			foreach ($this->post_data as $key => $row) {
				...
				if (!empty($row['value']) AND !is_array($row['value'])) {
					...
					if (preg_match('/float/', $row['rules']) && preg_match('/[^0-9\.]/', $row['value'])) {
						$_SESSION['temp']['error'][$key] = "$row[name] can only contain numbers";
					}
					...
					/* TEST FOR PASSWORD COMPLEXITY FIELDS */
					if (preg_match('/password/', $row['rules']) && (!preg_match('/((?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{6})/', $row['value']))) {
						$_SESSION['temp']['error'][$key] = "$row[name] must contain at least one upper case letter, one lower case letter and one digit and be at least six characters long";
					}
				}
			}
		}
